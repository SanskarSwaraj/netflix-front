...
const getRawData = async (api,genres,paging) => {
    const moviesArray = [];
    for(let i=1;moviesArray.length<60 && i<10; i++) // min number of movies is 60
    {
...
export const getUserLikedMovies = createAsyncThunk("netflix/getliked",async(email)=>{
    const {data:{movies},}=await axios.get(`https://netflixclonebackend-0kvk.onrender.com/api/user/liked/${email}`);
    return movies;
})

export const removeFromLikedMovies = createAsyncThunk("netflix/deleteLiked",async({email,movieId})=>{
    const {data:{movies},}=await axios.put(`https://netflixclonebackend-0kvk.onrender.com/api/user/delete`,{email,movieId});
    return movies;
})
